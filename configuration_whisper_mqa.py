...
""" Custom Whisper model configuration with MQA"""
...
class WhisperMQAConfig(PretrainedConfig):
    ...
    Args:
        ...
        attention_head_type (`str`, *optional*, defaults to `"multihead"`):
            The type of attention head. Can be either `"multihead"` or `"multiquery"`.


    Example:

    ```python
    >>> from whisper-mqa import WhisperCustomConfig, WhisperForConditionalGenerationCustom

    >>> # Initializing a Whisper tiny style configuration
    >>> configuration = WhisperCustomConfig()

    >>> # Initializing a custom Whisper model (with random weights) from the tiny style configuration
    >>> model = WhisperForConditionalGenerationCustom(configuration)
